...
- **4.2** — Persist settings in localStorage. Created `src/lib/settings-store.ts` with `readConfig`/`writeConfig` helpers and `useSprintConfig()` React hook. Hook returns `[config, setConfig]` tuple. Reads from localStorage on mount, writes through on every update. Validates stored JSON structure before accepting; falls back to `DEFAULT_CONFIG` on missing/invalid data or SSR. Hydration-safe (returns default until `useEffect` runs). TypeScript verified, all 32 tests passing.
...
| 4 | 4.2 — Persist settings in localStorage | done | `useSprintConfig()` hook, read/write localStorage, validation |
